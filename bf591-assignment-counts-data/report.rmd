...
filtered_counts <- filter_zero_var_genes(verse_counts)
...
sample_names <- colnames(filtered_counts)[-1]
meta <- meta_info_from_labels(sample_names)
meta
```
...
library_size <- get_library_size(filtered_counts)
#Make a barplot showing the number of millions of reads for each sample
barplot(library_size$value / 10^6, names.arg = library_size$sample, 
        main = "Number of Millions of Reads Per Sample", xlab = "Sample", ylab = "Millions of Reads")
```
...
cpm_normalized_counts <- normalize_by_cpm(filtered_counts)
head(cpm_normalized_counts)
```
...
deseq_normalized_counts <- deseq_normalize(filtered_counts, meta)
head(deseq_normalized_counts)
```
...
plot_sample_distributions(filtered_counts, scale_y_axis = TRUE, title = "Count Distributions")
```
...
plot_variance_vs_mean(filtered_counts, scale_y_axis = TRUE, title = "Variance vs Mean (Raw Counts)")
```
...
plot_pca(filtered_counts, meta, title = "Raw Count PCA")
```
...
plot_sample_distributions(cpm_normalized_counts, scale_y_axis = TRUE, title = "CPM Count Distributions")
plot_variance_vs_mean(cpm_normalized_counts, scale_y_axis = TRUE, title = "Variance vs Mean (CPM)")
plot_pca(cpm_normalized_counts, meta, title = "CPM PCA")
```
...
plot_sample_distributions(deseq_normalized_counts, scale_y_axis = TRUE, title = "DESeq2 Count Distributions")
plot_variance_vs_mean(deseq_normalized_counts, scale_y_axis = TRUE, title = "Variance vs Mean (DESeq2)")
plot_pca(deseq_normalized_counts, meta, title = "DESeq2 PCA")
```
